...
class CitizenPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `citizen`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM citizen WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO citizen (date, title, link, content)"
